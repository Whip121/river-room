...
export async function runHTTPChecks(origin='http://localhost:3000'){
  ...
  const full=await request('/api/rooms',{name:'满桌房主',avatar,settings:{capacity:2,buyIn:1000,smallBlind:5,bigBlind:10,turnSeconds:60,deckType:'short',tableShape:'round'},bots:1});const fullRoom=full.data.view.id;assert.equal(full.data.view.players[0].avatar,avatar);assert.equal(full.data.view.settings.tableShape,'round');
  const blockedName=await request(`/api/rooms/${fullRoom}/join`,{name:'某人的妈'});assert.equal(blockedName.status,400);
  ...
  const closed=await request(`/api/rooms/${fullRoom}/action`,{action:'adminClose'},watched.data.token);assert.equal(closed.status,200);assert.equal(closed.data.view.admin,false);assert.equal(closed.data.view.adminActive,false);
  return {room,checks:['双人独立会话加入','统一入场筹码','未登录访问拒绝','房主权限','双方底牌隔离','非法下注拒绝','重复操作拒绝','跨客户端轮流行动','会话恢复','房主转交','电脑添加与移除','满桌自动观战','管理员查看与关闭','短牌圆桌设置','头像地址','违规昵称拒绝']};
}
